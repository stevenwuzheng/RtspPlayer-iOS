...
NSString * const KxMovieDidFinishLoadStream_Notify = @"KxMovieDidFinishLoadStream_Notify";
...
#define LOCAL_MIN_BUFFERED_DURATION   0.5 //0.2
    ...
    if (!error && decoder) {
        ...
        if (self.isViewLoaded) {
            ...
            if (_activityIndicatorView.isAnimating) {
                ...
                [_activityIndicatorView stopAnimating];
                //加载实时流完毕(wz2017.2.24)
                [[NSNotificationCenter defaultCenter] postNotificationName:KxMovieDidFinishLoadStream_Notify object:nil];
                // if (self.view.window)
                [self restorePlay];
            }
        }
...
#pragma mark 自定义手势
    //隐藏工具栏手势
    view.tag = 1003;
    UITapGestureRecognizer *hideBarGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapAction)];
    [view addGestureRecognizer:hideBarGesture];
}
    
-(void)tapAction {
    if (_tapMovieScreenBlock) _tapMovieScreenBlock();
}
...
//                                 lineBreakMode:NSLineBreakByTruncatingTail];
                CGSize size = [ms boundingRectWithSize:CGSizeMake(viewSize.width, viewSize.height * 0.5) options:NSStringDrawingUsesLineFragmentOrigin attributes:nil context:nil].size;
...
- (void)restoreVideo
{
    [self restorePlayWithPath:_decoder.path];
}
